...
func (result *result) CurrentTemperature() float64 {
	...
	return result.Hourly.Temperature[rand.Intn(len(result.Hourly.Temperature))]

}

func (result *result) CurrentWind() float64 {
	rand.Seed(time.Now().Unix())
	return result.Hourly.Windspeed[rand.Intn(len(result.Hourly.Windspeed))]

}

func (result *result) CurrentRain() float64 {
	rand.Seed(time.Now().Unix())
	return result.Hourly.Rain[rand.Intn(len(result.Hourly.Rain))]
